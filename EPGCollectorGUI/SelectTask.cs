...
namespace EPGCentre
{
    internal partial class SelectTask : Form
    {
        ...
        internal SelectTask(bool pluginList) : base()
        {
            InitializeComponent();
            ...
            {
                Logger.Instance.Write("The preset tasks were not successfully loaded");
                Logger.Instance.Write(e.Message);
            }

            if (lbTasks.Items.Count == 0)
                MessageBox.Show("The preset tasks could not be loaded", "EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Error);
