...
namespace EPGCentre
{
    internal partial class ViewSatIPControl : UserControl, IView
    {
        internal bool FindFilterAvailability { get { return (records.Count != 0); } }
        ...
        public ViewSatIPControl()
        {
            InitializeComponent();
        }

        internal void Process(string logFileName, Logger logger)
        {
            ...
            for (; checkIndex(index, lastIndex, findText.SearchDown); index += increment)
            {
                ...
                foreach (string searchString in searchStrings)
                {
                    if (!findText.IgnoreCase)
                    {
                        ...
                        if (record.Time.Contains(editedString) || record.Detail.Contains(editedString))
                        {
                    ...
                    else
                    {
                        ...
                        if (record.Time.ToUpper().Contains(editedString) || record.Detail.ToUpper().Contains(editedString))
                        {
                            dgViewLog.FirstDisplayedCell = dgViewLog.Rows[records.IndexOf(record)].Cells[0];
                            return;
                        }
        ...
        private bool checkLine(LineEntry lineEntry, string[] filterStrings, bool ignoreCase)
        {
            foreach (string filterString in filterStrings)
            {
                if (!ignoreCase)
                {
                    if (lineEntry.Time.Contains(filterString.Trim()) || lineEntry.Detail.Contains(filterString.Trim()))
                        return (true);
                }
                else
                {
                    string upperFilterString = filterString.Trim().ToUpper();
                    if (lineEntry.Time.ToUpper().Contains(upperFilterString) || lineEntry.Detail.ToUpper().Contains(upperFilterString))
                        return (true);
                }
            }
