...
namespace EPGCentre
{
    internal partial class ChangeSdCategoriesControl : BaseChangeCategoryControl, IUpdateControl
    {
        /// <summary>
        /// Get the general window heading for the data.
        /// </summary>
        public string Heading { get { return ("EPG Centre - Change Schedules Direct Program Categories - "); } }
        /// <summary>
        ...
        /// </summary>
        public string DefaultFileName { get { return ("Schedules Direct Categories"); } }
        /// <summary>
        /// Get the save file filter.
        /// </summary>
        public string SaveFileFilter { get { return ("Schedules Direct Program Category Files (Schedules Direct Categories*.cfg)|Schedules Direct Categories*.cfg"); } }
        /// <summary>
        /// Get the save file title.
        /// </summary>
        public string SaveFileTitle { get { return ("Save EPG Collection Schedules Direct Program Category File"); } }
        /// <summary>
        /// Get the save file suffix.
        /// </summary>
        public string SaveFileSuffix { get { return ("cfg"); } }

        internal ChangeSdCategoriesControl()
        {
            InitializeComponent();
        }
        ...
        {
            if (!SchedulesDirectProgramCategory.Load(fileName))
                return (false);
            ...
            foreach (ProgramCategory category in SchedulesDirectProgramCategory.Categories)
        ...
        protected override DataState HasDataChanged()
        {
            ...
            if (BindingList.Count != SchedulesDirectProgramCategory.Categories.Count)
                return (DataState.Changed);

            foreach (ProgramCategory category in BindingList)
            {
                ProgramCategory existingCategory = SchedulesDirectProgramCategory.FindCategory(category.CategoryTag);
        ...
        public bool PrepareToSave()
        {
            ...
            SchedulesDirectProgramCategory.Categories.Clear();

            foreach (ProgramCategory category in BindingList)
            {
                if (!category.IsEmpty)
                    SchedulesDirectProgramCategory.Categories.Add(category);
            }
        ...
        public bool Save(string fileName)
        {
            ...
            string message = SchedulesDirectProgramCategory.Save(fileName);

            if (message == null)
            {
                MessageBox.Show("The Schedules Direct program categories have been saved to '" + fileName + "'", " EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Information);
                CurrentFileName = fileName;
            }
            else
                MessageBox.Show("An error has occurred while writing the Schedules Direct program categories." + Environment.NewLine + Environment.NewLine + message, "EPG Centre", MessageBoxButtons.OK, MessageBoxIcon.Error);
