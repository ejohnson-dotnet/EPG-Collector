...
namespace Lookups
{
    ...
    public sealed class LookupController
    {
            ...
            else
            {
                ...
                else
                {
                    if (RunParameters.Instance.MovieLookupEnabled)
                    {
                        ...
                        Logger.Instance.Write("TV lookup processing disabled");
                    }
                    else
                    {
                        Logger.Instance.Write("Movie lookup processing disabled");
        ...
        internal static bool DeletePoster(string type, string posterId, string originalTitle)
        {
            string posterPath = Path.Combine(RunParameters.ImagePath, type, posterId + ".jpg");
            bool deleted = deletePoster(posterPath);
            if (deleted)
                return (true);

            posterPath = Path.Combine(RunParameters.ImagePath, posterId + ".jpg");
