...
namespace Lookups
{
    internal class MovieLookup
    {
        ...
        internal MovieLookup(Logger streamLogger, string logHeader) 
        {
        ...
        internal LookupController.LookupReply Process(EPGEntry epgEntry)
        {
            ...
            if (checkInNotMovieList(epgEntry.EventName))
                return (LookupController.LookupReply.NotMovie);
            if (checkInNotMovieList(searchTitle))
                return (LookupController.LookupReply.NotMovie);
        ...
        private bool checkInNotMovieList(string title)
        {
            ...
            foreach (string notMovieTitle in RunParameters.Instance.LookupNotMovie)
            {
                if (lowerCaseTitle == notMovieTitle.ToLowerInvariant())
                    return true;
            }
        ...
        private MovieEntry createMovieEntry(string title, TmdbMovie movie, string originalTitle)
        {
            ...
            try
            {
                ...
                {
                    foreach (TmdbGenre genre in movie.Genres)
                    {
                        if (movieEntry.Genre == null)
                            movieEntry.Genre = genre.Name;
                        else
                            movieEntry.Genre += "," + genre.Name;
                    }
                }
                ...
                {                    
                    ...
                    posterPath = Path.Combine(imageDirectory, imageId + ".jpg");
                    
                    bool imageLoaded = movie.GetPosterImage(apiInstance, posterPath);
                    if (imageLoaded)
                    {
                        movieEntry.Poster = imageId;
                        postersDownloaded++;
                        if (TraceEntry.IsDefined(TraceName.Lookups))
        ...
        private void processMovie(EPGEntry epgEntry, MovieEntry movieEntry)
        {
            ...
            {
                if (!string.IsNullOrWhiteSpace(movieEntry.Overview))
                {
            ...
            if (movieEntry.Cast != null && movieEntry.Cast.Count != 0)
            {
                ...
                foreach (string castMember in movieEntry.Cast)
            ...
            if (movieEntry.Producers != null && movieEntry.Producers.Count != 0)
            {
                ...
                foreach (string producer in movieEntry.Producers)
            ...
            if (movieEntry.Directors != null && movieEntry.Directors.Count != 0)
            {
                ...
                foreach (string director in movieEntry.Directors)
            ...
            if (movieEntry.Writers != null && movieEntry.Writers.Count != 0)
            {
                ...
                foreach (string writer in movieEntry.Writers)
        ...
        private void createEventCategory(EPGEntry epgEntry, MovieEntry movieEntry)
        {
            ...
            LookupCategory movieCategory = new LookupCategory();
            movieCategory.XmltvDescription = "Movie," + movieEntry.Genre;
            movieCategory.WMCDescription = "Movies," + movieEntry.Genre + ",isMovie";
            movieCategory.DVBLogicDescription = "movie";
            movieCategory.DVBViewerDescription = "1,0";             
            epgEntry.EventCategory = new EventCategorySpec(movieCategory);            
        }
        ...
        internal void LogStats()
        {
            ...
            if (apiInstance.TotalRequestCount != 0)
            {
                ...
                Logger.Instance.Write("Movie statistics: Posters downloaded = " + postersDownloaded);
            }
