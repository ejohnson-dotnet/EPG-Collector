...
namespace Lookups
{
    internal class TmdbTVLookup : TVLookupBase
    {
        ...
        internal TmdbTVLookup(Logger streamLogger, string logHeader, TmdbAPI currentInstance) : base(streamLogger, logHeader)
        {
            ...
            if (currentInstance == null)
            {
                try
                {
                    apiInstance = new TmdbAPI("b5410cd85abf11ab7e32d6addd5d5963", streamLogger, logHeader);
                    Initialized = true;

                    Logger.Instance.Write("TMDB TV Lookup initialized");
                }
                catch (Exception e)
                {
            ...
            {
                apiInstance = currentInstance;
                Initialized = true;
                Logger.Instance.Write("TMDB TV Lookup using exisitng instance");
            }
        ...
        internal override LookupController.LookupReply Process(EPGEntry epgEntry)
        {
            ...
            try
            {
                ...
                {
                    TmdbSeriesSearchResults results;
                    ...
                    results = TmdbSeries.Search(apiInstance, searchTitle, languageCode);
                ...
                {
                    if (TraceEntry.IsDefined(TraceName.Lookups))
                        Logger.Instance.Write("Found series " + tvSeriesEntry.Name);

                    if (!fromCache)
                    {
                        tvSeriesEntry.LoadDetails(apiInstance);
        ...
        private TmdbSeries findTVSeriesEntry(Collection<TmdbSeries> series, string title, string subTitle)
        {
            if (!string.IsNullOrWhiteSpace(subTitle))
            {
                TmdbSeries episodeSeries = findTVSeriesEntryUsingSubTitle(series, title, subTitle);
            ...
            foreach (TmdbSeries tvSeriesEntry in series)
            {
                if (!string.IsNullOrWhiteSpace(tvSeriesEntry.Name))
                {
                    if (tvSeriesEntry.Name.ToLowerInvariant() == noCaseTitle + " (" + noCaseRegionCode + ")" ||
                        tvSeriesEntry.Name.ToLowerInvariant() == noCaseTitle + " " + noCaseRegionCode ||
                        tvSeriesEntry.Name.ToLowerInvariant() == noCaseTitle + " " + noCaseRegionName)
                        return tvSeriesEntry;
                }
            }
            ...
            foreach (TmdbSeries tvSeriesEntry in series)
            {
                if (!string.IsNullOrWhiteSpace(tvSeriesEntry.Name))
                {
                    if (tvSeriesEntry.Name.ToLowerInvariant() == noCaseTitle)
                        return tvSeriesEntry;
                }
            }
            ...
            switch (RunParameters.Instance.LookupMatching)
            {
                ...
                case MatchMethod.Contains:
                    TmdbSeries matchedEntry = null;

                    foreach (TmdbSeries tvSeriesEntry in series)
                    {
                        if (!string.IsNullOrWhiteSpace(tvSeriesEntry.Name))
                        {
                            if (tvSeriesEntry.Name.ToLowerInvariant().Contains(noCaseTitle))
                            {
                                if (matchedEntry == null)
                                    matchedEntry = tvSeriesEntry;
                                else
                                {
                                    int lengthDiff1 = title.Length - matchedEntry.Name.Length;
                                    if (lengthDiff1 < 0)
                                        lengthDiff1 *= -1;

                                    int lengthDiff2 = title.Length - tvSeriesEntry.Name.Length;
                                    if (lengthDiff2 < 0)
                                        lengthDiff2 *= -1;
                ...
                case MatchMethod.Nearest:
                    ...
                    {
                        if (!string.IsNullOrWhiteSpace(tvSeriesEntry.Name))
                            titleList.Add(tvSeriesEntry.Name);
                    }
                    ...
                    if (RunParameters.Instance.LookupMatchThreshold == 0)
                        return series[matchResult.Index];
                    else
                    {
        ...
        private TmdbSeries findTVSeriesEntryUsingSubTitle(Collection<TmdbSeries> series, string title, string subTitle)
        {
            if (string.IsNullOrWhiteSpace(subTitle))
                return (null);
            ...
            foreach (TmdbSeries tvSeriesEntry in series)
            {
                ...
                if (tvSeriesEntry.Seasons != null)
                {
                    foreach (TmdbSeason season in tvSeriesEntry.Seasons)
                    {
                        if (season.Episodes != null)
                        {
                            foreach (TmdbEpisode episode in season.Episodes)
                            {
                                ...
                                {
                                    if (TraceEntry.IsDefined(TraceName.Lookups))
                                        Logger.Instance.Write("Series " + tvSeriesEntry.Name + " located with episode " + episode.EpisodeName);
                                    return (tvSeriesEntry);
                                }
                            }
                        }
            ...
            double bestResult = -1;
            TmdbSeries bestSeries = null;
            TmdbEpisode bestEpisode = null;
                            ...
                            foreach (TmdbEpisode episode in season.Episodes)
                            {
                                ...
                                else
                                {
                                    if (TraceEntry.IsDefined(TraceName.LookupsError))
                                        Logger.Instance.Write("Series " + tvSeriesEntry.Name + " episode " + episode.EpisodeNumber + " has no episode name");
                                }
                            }
                        }
                    }
            ...
            if (TraceEntry.IsDefined(TraceName.Lookups))
                Logger.Instance.Write("Best match was " + bestResult.ToString("0.000") + " episode " +
                    bestEpisode.EpisodeName + " in series " + bestSeries.Name);
                        ...
                        if (season.Episodes != null)
                        {
                            foreach (TmdbEpisode episode in season.Episodes)
                            {
                                if (!string.IsNullOrWhiteSpace(episode.EpisodeName))
                                {
                                    ...
                                    {
                                        if (TraceEntry.IsDefined(TraceName.Lookups))
                                            Logger.Instance.Write("Series " + tvSeriesEntry.Name + " partial match with episode " + episode.EpisodeName);
        ...
        private void addToCacheEntries(string title, TmdbSeries series)
        {
            if (cacheEntries == null)
        ...
        private TmdbSeries findCacheEntry(string title, string subTitle)
        {
            foreach (CacheEntry cacheEntry in cacheEntries)
            {
                if (cacheEntry.Title == title)
                {
                    ...
                    if (cacheEntry.Series.Seasons != null)
                    {
                        foreach (TmdbSeason season in cacheEntry.Series.Seasons)
                        {
                            if (season.Episodes != null)
                            {
                                foreach (TmdbEpisode episode in season.Episodes)
        ...
        private TVSeriesEntry createTVSeriesEntry(string searchTitle, TmdbSeries tvSeriesEntry, EPGEntry epgEntry, bool fromCache)
        {
            ...
            try
            {
                seriesEntry = new TVSeriesEntry(searchTitle, tvSeriesEntry.Name);
                seriesEntry.OriginalTitle = epgEntry.EventName;

                string seriesOverview = tvSeriesEntry.Overview;                
                if (seriesOverview != null)
                    seriesEntry.SeriesOverview = LookupController.ReplaceHtmlChars(seriesEntry.Overview);
                
                seriesEntry.SeriesStartDate = tvSeriesEntry.FirstAirDate;
                ...
                seriesEntry.StarRating = LookupController.GetStarRating(tvSeriesEntry.VoteAverage == null ? 0 : (decimal)tvSeriesEntry.VoteAverage);

                if (tvSeriesEntry.Genres != null && tvSeriesEntry.Genres.Count > 0)
                {
                    foreach (TmdbGenre genre in tvSeriesEntry.Genres)
                    {
                ...
                TmdbEpisode episode = findEpisode(tvSeriesEntry, epgEntry.SeasonNumber, epgEntry.EpisodeNumber, epgEntry.EventName, epgEntry.EventSubTitle);
                ...
                if (episode != null)
                {
                    ...
                    episode.LoadLists(apiInstance, tvSeriesEntry.Identity, epgEntry.SeasonNumber);
                    ...
                    if (episode.Cast != null && episode.Cast.Cast != null)
                        seriesEntry.Cast = episode.Cast.CastNames;
                    else
                    {
                        if (tvSeriesEntry.Cast != null && tvSeriesEntry.Cast.Cast != null)
                            seriesEntry.Cast = tvSeriesEntry.Cast.CastNames;
                    }

                    if (episode.Cast != null && episode.Cast.Crew != null)
                    {
                        seriesEntry.Directors = episode.Cast.DirectorNames;
                        seriesEntry.Writers = episode.Cast.WriterNames;
                    }
                    else
                    {
                        if (tvSeriesEntry.Cast != null && tvSeriesEntry.Cast.Crew != null)
                        {
                            seriesEntry.Directors = tvSeriesEntry.Cast.DirectorNames;
                            seriesEntry.Writers = tvSeriesEntry.Cast.WriterNames;
                        }
                    }

                    if (episode.Cast != null && episode.Cast.GuestStars != null)
                    {
                        foreach (TmdbCastMember guestStar in episode.Cast.GuestStars)
                        {
                            if (seriesEntry.Cast == null || (seriesEntry.Cast != null && !seriesEntry.Cast.Contains(guestStar.Name)))
                            {
                                if (seriesEntry.GuestStars == null)
                                    seriesEntry.GuestStars = new Collection<string>();
                                seriesEntry.GuestStars.Add(guestStar.Name);
                            }
                        }
                    }
                    
                    seriesEntry.FirstAiredDate = episode.AirDate;
                }
                else
                {
                    if (TraceEntry.IsDefined(TraceName.Lookups))
        ...
        private void loadImage(string title, string searchTitle, TVSeriesEntry seriesEntry, TmdbSeries tvSeriesEntry)
        {
                ...
                try
                {
                    switch (RunParameters.Instance.DownloadTVThumbnail)
                    {
                        ...
                        case LookupImageType.Banner:
                            imageLoaded = tvSeriesEntry.GetBackdropImage(apiInstance, posterPath);
                            break;
                        default:
                            imageLoaded = tvSeriesEntry.GetPosterImage(apiInstance, posterPath);
                            break;
        ...
        private TmdbEpisode findEpisode(TmdbSeries series, int seasonNumber, int episodeNumber, string title, string subtitle)
        {
            if (series.Seasons == null || seasonNumber < 1 || seasonNumber > series.Seasons.Count || series.Seasons[seasonNumber - 1].Identity == 0)
                return null;
        ...
        private TmdbEpisode findEpisodeUsingNumbers(TmdbSeries series, int seasonNumber, int episodeNumber)
        {
            foreach (TmdbSeason season in series.Seasons)
            {
        ...
        private TmdbEpisode findEpisodeUsingSubtitle(TmdbSeries series, string subtitle)
        {
            if (string.IsNullOrWhiteSpace(subtitle))
                return null;
            ...
            foreach (TmdbSeason season in series.Seasons)
            {
                if (season.Episodes != null)
                {
                    foreach (TmdbEpisode episode in season.Episodes)
                    {
                        ...
                        {
                            if (TraceEntry.IsDefined(TraceName.LookupsError))
                                Logger.Instance.Write("Series " + series.Name + " episode " + episode.EpisodeNumber + " has no episode name");
                        }
                    }
                }
            }
        ...
        private class CacheEntry
        {
            ...
            internal TmdbSeries Series { get; private set; }
            
            private CacheEntry() { }

            internal CacheEntry(string title, TmdbSeries series)
            {
                Title = title;
                Series = series;
            }
