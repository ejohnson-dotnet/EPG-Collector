...
namespace NetworkProtocols
{
    ...
    public sealed class Utils
    {
        ...
        /// <param name="serverType">The type of streaming server.</param>
        /// <returns></returns>
        public static IPAddress GetLocalIpAddress(ILogger logger, StreamServerType serverType)
        {
            IPAddress localAddress = getUserSpecifiedAddress(serverType);
            if (localAddress != null)
            {
                logger.Write("Using user specified IP Address (" + serverType + ") " + localAddress);
                return (localAddress);
            }
        ...
        private static IPAddress getUserSpecifiedAddress(StreamServerType serverType)
        {
            if (serverType == StreamServerType.SatIP)
            {
                if (NetworkConfiguration.SatIpConfiguration == null || NetworkConfiguration.SatIpConfiguration.LocalAddress == null)
                    return (null);
                else
                    return (IPAddress.Parse(NetworkConfiguration.SatIpConfiguration.LocalAddress));
            }

            if (serverType == StreamServerType.VBox)
            {
                if (NetworkConfiguration.VBoxConfiguration == null || NetworkConfiguration.VBoxConfiguration.LocalAddress == null)
                    return (null);
                else
                    return (IPAddress.Parse(NetworkConfiguration.VBoxConfiguration.LocalAddress));
            }
