...
namespace DomainObjects
{
    ...
    public abstract class ImportFileBase
    {
        ...
        public void MergeChannels(Collection<TVStation> newChannels, DataPrecedence precedence)
        {
            ...
            foreach (TVStation newStation in newChannels)
            {
                ...
                foreach (TVStation existingStation in RunParameters.Instance.StationCollection)
                {
                    if (existingStation.Included)
                    {
                        ...
                        if (merged)
                        {
                            mergeChannel(existingStation, newStation, precedence);
        ...
        private bool matchChannels(TVStation existingStation, TVStation newStation)
        {
            ...
            {
                if (!newStation.IgnoreFrequencyForMerge)
                {
                    if (existingStation.OriginalNetworkID != -1 && newStation.OriginalNetworkID != -1)
                    {
                    ...
                    {
                        if (existingStation.TransportStreamID != -1 && newStation.TransportStreamID != -1)
                        {
                            return (existingStation.TransportStreamID == newStation.TransportStreamID &&
                                existingStation.ServiceID == newStation.ServiceID);
                        }
                        else
                            return (existingStation.ServiceID == newStation.ServiceID);
                    }
                }
                else
                {
                    // Note this has been added for Zap2it/Schedules Direct ATSC import. Major channel is in TSID, minor channel is in SID.
                    return (existingStation.TransportStreamID == newStation.TransportStreamID &&
                        existingStation.ServiceID == newStation.ServiceID);
                }
            }
        ...
        private void mergeChannel(TVStation oldStation, TVStation newStation, DataPrecedence precedence)
        {
            ...
            if (precedence == DataPrecedence.ImportAppend)
            {
            ...
            if (precedence == DataPrecedence.ImportReplace)
            {
                oldStation.EPGCollection = new Collection<EPGEntry>();
            ...
            Collection<EPGEntry> combinedEPGCollection = new Collection<EPGEntry>();

            while (oldIndex < oldStation.EPGCollection.Count || newIndex < newStation.EPGCollection.Count)
            {
                if (oldIndex < oldStation.EPGCollection.Count)
                {
                    if (newIndex < newStation.EPGCollection.Count)
                    {
                        DateTime oldTime = oldStation.EPGCollection[oldIndex].StartTime;
                        DateTime oldStartTime = new DateTime(oldTime.Year, oldTime.Month, oldTime.Day, oldTime.Hour, oldTime.Minute, 0);

                        DateTime newTime = newStation.EPGCollection[newIndex].StartTime;
                        DateTime newStartTime = new DateTime(newTime.Year, newTime.Month, newTime.Day, newTime.Hour, newTime.Minute, 0);

                        if (oldStartTime == newStartTime)
                        {
                            combinedEPGCollection.Add(mergeProgramme(oldStation.EPGCollection[oldIndex], newStation.EPGCollection[newIndex], precedence));
                            oldIndex++;
                            newIndex++;
                        }
                        else
                        {
                            ...
                            {
                                combinedEPGCollection.Add(oldStation.EPGCollection[oldIndex]);
                                oldIndex++;
                            }
                            else
                            {
                                combinedEPGCollection.Add(newStation.EPGCollection[newIndex]);
                                newIndex++;
                            }
                        }
                    }
            ...
            oldStation.EPGCollection = combinedEPGCollection;
        }

        private EPGEntry mergeProgramme(EPGEntry oldProgramme, EPGEntry newProgramme, DataPrecedence precedence)
        {
            if (precedence == DataPrecedence.File)
            {
                oldProgramme.StartTime = newProgramme.StartTime;
                oldProgramme.Duration = newProgramme.Duration;
            }
