...
namespace DomainObjects
{
    ...
    public class EditSpec
    {
        ...
        {
            string actualText = editSpec.ActualText.Replace("|", ",").ToLowerInvariant();            
            ...
            switch (editSpec.Location)
            {
                case TextLocation.Start:
                    if (editedText.ToLowerInvariant().StartsWith(actualText))
                    {
                        editedText = editedText.Remove(0, actualText.Length);
                        
                        if (editSpec.ActualReplacementText != null)
                        {
                            string actualReplacementText = editSpec.ReplacementText.Replace("|", ",");

                            switch (editSpec.ReplacementMode)
                            {
                                case TextReplacementMode.TextOnly:
                                case TextReplacementMode.TextAndPreceeding:
                                    editedText = actualReplacementText + editedText;
                                    break;
                                case TextReplacementMode.TextAndFollowing:
                                case TextReplacementMode.Everything:
                                    editedText = actualReplacementText;
                                    break;
                                default:
                                    editedText = actualReplacementText + editedText;
                                    break;
                ...
                case TextLocation.End:
                    if (editedText.ToLowerInvariant().EndsWith(actualText))
                    {
                        editedText = editedText.Remove(editedText.Length - actualText.Length);                        
                        ...
                        if (editSpec.ActualReplacementText != null)
                        {
                            ...
                            switch (editSpec.ReplacementMode)
                            {
                                case TextReplacementMode.TextOnly:
                                case TextReplacementMode.TextAndFollowing:
                                    editedText = editedText + actualReplacementText;
                                    break;
                                case TextReplacementMode.TextAndPreceeding:
                                case TextReplacementMode.Everything:
                                    editedText = actualReplacementText;
                ...
                case TextLocation.Anywhere:
                    ...
                    while (!done)
                    {
                        done = startIndex >= editedText.Length;
                        if (!done)
                        {
                            startIndex = editedText.ToLowerInvariant().IndexOf(actualText, startIndex);
                            done = startIndex == -1;
                        }
                        ...
                        if (!done)
                        {
                            if (editSpec.ActualReplacementText != null)
                            {
                                ...
                                {
                                    case TextReplacementMode.TextOnly:
                                        editedText = editedText.Substring(0, startIndex) + actualReplacementText + editedText.Substring(startIndex + actualText.Length);
                                        break;
                                    case TextReplacementMode.TextAndFollowing:
                                        editedText = editedText.Substring(0, startIndex) + actualReplacementText;
                                        done = true;
                                        break;
                                    case TextReplacementMode.TextAndPreceeding:
                                        editedText = actualReplacementText + editedText.Substring(startIndex + actualText.Length);
                                        break;
                                    case TextReplacementMode.Everything:
                                        editedText = actualReplacementText;
                                        done = true;
                            ...
                            else
                            {
                                ...
                                {
                                    case TextReplacementMode.TextOnly:
                                        editedText = editedText.Remove(startIndex, actualText.Length);
                                        break;
                                    case TextReplacementMode.TextAndFollowing:
                                        editedText = editedText.Remove(startIndex);
                                        done = true;
                                        break;
                                    case TextReplacementMode.TextAndPreceeding:
                                        editedText = editedText.Substring(startIndex + actualText.Length);
                                        break;
                                    case TextReplacementMode.Everything:
                                        editedText = null;
                                        done = true;
