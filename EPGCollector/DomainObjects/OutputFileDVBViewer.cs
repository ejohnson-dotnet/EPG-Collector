    ...
    public class OutputFileDVBViewer
    {
        ...
        public static string Process()
        {
            ...
            if (OptionEntry.IsDefined(OptionName.DvbViewerImport))
                reply = processDVBViewer();
            else
                reply = processRecordingService();
        ...
        private static int processStationEPG(IEPGAddBuffer epgAddBuffer, TVStation tvStation)
        {
            ...
            {
                if (OptionEntry.IsDefined(OptionName.NoInvalidEntries) && epgEntry.Duration.TotalSeconds < 1)
                    invalidTimes++;
                else
                {
                    ...
                    if (epgEntry.EventCategory != null)
                    {
                        try
                        {
                            newItem.Content = Int32.Parse(epgEntry.EventCategory.GetDescription(EventCategoryMode.DvbViewer).Trim());
                        }
        ...
        private static int processStationEPG(XmlWriter xmlWriter, TVStation tvStation)
        {
            ...
            foreach (EPGEntry epgEntry in tvStation.EPGCollection)
            {
                ...
                else
                {
                    ...
                    if (epgEntry.EventID != 0)
                        xmlWriter.WriteElementString("eventid", epgEntry.EventID.ToString());
                    else
                        xmlWriter.WriteElementString("eventid", (epgCount + 1).ToString());

                    if (epgEntry.EventCategory != null)
                    {
                        try
                        {
                            string eventDescription = epgEntry.EventCategory.GetDescription(EventCategoryMode.DvbViewer);
                            if (!string.IsNullOrWhiteSpace(eventDescription))
                            {
                                string[] parts = eventDescription.Split(new char[] { ',' });
                    ...
                    if (!OptionEntry.IsDefined(OptionName.DvbViewerSubtitleVisible))
                    {
                        ...
                        if (epgEntry.ShortDescription != null)
                        {
                            if (epgEntry.EventSubTitle == null)
                                ...
                                xmlWriter.WriteValue(epgEntry.EventSubTitle + ": " + epgEntry.ShortDescription);
                        }
                        else
                        {
                            if (epgEntry.EventName != null)
