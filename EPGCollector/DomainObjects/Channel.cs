...
namespace DomainObjects
{
    ...
    public class Channel
    {
        ...
        public static void LogChannelsInChannelIDOrder()
        {
            ...
            Logger.Instance.WriteSeparator("Channel List - Channel ID Order");
        ...
        /// </summary>
        public static void LogChannelsInChannelNetworkOrder()
        {
            Collection<Channel> sortedChannels = new Collection<Channel>();

            foreach (Channel channel in Channels)
                addInChannelNetworkOrder(channel, sortedChannels);

            Logger.Instance.WriteSeparator("Channel List - Network Order");

            foreach (Channel channel in sortedChannels)
                Logger.Instance.Write(channel.ToString());
        }

        private static void addInChannelNetworkOrder(Channel newChannel, Collection<Channel> sortedChannels)
        {
            foreach (Channel oldChannel in sortedChannels)
            {
                if (oldChannel.OriginalNetworkID > newChannel.OriginalNetworkID)
        ...
        /// </summary>
        public static void LogChannelsInServiceIdOrder()
        {
            Collection<Channel> sortedChannels = new Collection<Channel>();

            foreach (Channel channel in Channels)
                addInChannelServiceIdOrder(channel, sortedChannels);

            Logger.Instance.WriteSeparator("Channel List - Service ID Order");

            foreach (Channel channel in sortedChannels)
                Logger.Instance.Write(channel.ToString());
        }

        private static void addInChannelServiceIdOrder(Channel newChannel, Collection<Channel> sortedChannels)
        {
            foreach (Channel oldChannel in sortedChannels)
            {
                if (oldChannel.ServiceID >= newChannel.ServiceID)
