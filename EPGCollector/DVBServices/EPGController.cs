...
namespace DVBServices
{
    ...
    public sealed class EPGController
    {
        ...
        public bool FinishRun()
        {
            ...
            if (RunParameters.Instance.ImportFiles != null)
            {
                ...
                foreach (ImportFileSpec importFileSpec in RunParameters.Instance.ImportFiles)
                {
                    ...
                    importFileController.LogStats();
                }

                Logger.Instance.WriteSeparator("End Of XMLTV/MXF Import Processes");
            }

            if (RunParameters.Instance.AbandonRequested)
                return false;

            if (RunParameters.Instance.SdChannels != null)
            {
                Logger.Instance.WriteSeparator("Start Of Schedules Direct Import Processes");

                string configReply = SchedulesDirectConfig.Instance.Load();
                if (configReply != null)
                {
                    Logger.Instance.Write("Failed to load the Schedules Direct configuration parameters: " + configReply);
                    return false;
                }
                ...
                {
                    Logger.Instance.Write("Failed to initialize Schedules Direct service: " + reply.Message);
                    return false;
                }
                else
                {
                    int notDeleted = 0;
                    foreach (SchedulesDirectChannel channel in RunParameters.Instance.SdChannels)
                    {
                        if (!channel.Excluded)
                            notDeleted++;
                    }

                    Collection<SchedulesDirectChannel> collectionList = new Collection<SchedulesDirectChannel>();
                    int currentNumber = 1;
                    int groupCount = 50;
                    int totalEpgCount = 0;
                    ...
                    foreach (SchedulesDirectChannel channel in RunParameters.Instance.SdChannels)
                    {
                        ...
                        if (!channel.Excluded)
                        {
                            if (collectionList.Count == groupCount)
                            {
                                Logger.Instance.Write("Processing channel " + currentNumber + " to " + (currentNumber + groupCount - 1) + " of " + notDeleted + " channels");
                                reply = SchedulesDirectController.Instance.LoadEpg(collectionList);
                                if (reply.Message != null)
                                {
                                    Logger.Instance.Write("Failed to load EPG data: " + reply.Message);
                                    collectionList.Clear();
                                    break;
                                }
                                else
                                {
                                    totalEpgCount += (int)reply.ResponseData;
                                    collectionList.Clear();
                                }

                                currentNumber += groupCount;
                            }
                            else
                                collectionList.Add(channel);
                        }
                    }

                    if (collectionList.Count != 0)
                    {
                        Logger.Instance.Write("Processing channel " + currentNumber + " to " + (currentNumber + collectionList.Count - 1) + " of " + notDeleted + " channels");
                        reply = SchedulesDirectController.Instance.LoadEpg(collectionList);
                        if (reply.Message != null)
                            Logger.Instance.Write("Failed to load EPG data: " + reply.Message);
                        else
                    ...
                    Logger.Instance.Write("Total channels = " + notDeleted);
                    Logger.Instance.Write("Total EPG count = " + totalEpgCount);
                }

                SchedulesDirectController.Instance.LogStats();
                
                Logger.Instance.WriteSeparator("End Of Schedules Direct Import Processes");
            }
        ...
        {
            if (!OptionEntry.IsDefined(OptionName.DuplicateSameChannels) && !OptionEntry.IsDefined(OptionName.DuplicateSameChannelsNoData))
                return;
            ...
            foreach (TVStation station in sortedStations)
            {
                ...
                else
                {
                    ...
                    {
                        if (OptionEntry.IsDefined(OptionName.DuplicateSameChannels) || (OptionEntry.IsDefined(OptionName.DuplicateSameChannelsNoData) && station.EPGCollection.Count == 0))
                        {
                    ...
                    if (reader.IsStartElement())
                    {
                        switch (reader.Name)
                        {
                            ...
                            case "atscTuningInfo":
                                ...
                                break;
                            case "digitalCableTuningInfo":
                                channel.LoadDigitalCableTuningInfo(reader);
                                break;
                            case "recording":
        ...
        private void processStation(TVStation station, Collection<TVStation> manualMatchList)
        {
            foreach (WMCChannel channel in WMCChannel.Channels)
            {
                foreach (WMCTuningInfo tuningInfo in channel.TuningInfos)
                {
                    ...
                    {
                        if (station.OriginalNetworkID == dvbTuningInfo.ONID &&  station.TransportStreamID == dvbTuningInfo.TSID && station.ServiceID == dvbTuningInfo.SID)
                        {
                            if (setWMCStationInfo(station, channel, "tuning info"))
                            {
                        ...
                        else
                        {
                            ...
                            {
                                if (setWMCStationInfo(station, channel, "DVBLink data and tuning info"))
                                {
                                    if (manualMatchList.Contains(station))
                                        manualMatchList.Remove(station);
                                    return;
                    ...
                    else
                    {
                        ...
                        {
                            if (station.TransportStreamID == atscTuningInfo.MajorChannel && station.ServiceID == atscTuningInfo.MinorChannel)
                            {
                                if (setWMCStationInfo(station, channel, "tuning info"))
                                {
                                    if (manualMatchList.Contains(station))
                        ...
                        {
                            WMCDigitalCableTuningInfo digitalCableTuningInfo = tuningInfo as WMCDigitalCableTuningInfo;
                            if (digitalCableTuningInfo != null)
                            {
                                if (station.ServiceID == digitalCableTuningInfo.ChannelNumber)
                                {
                                    if (setWMCStationInfo(station, channel, "tuning info"))
                                    {
                                        if (manualMatchList.Contains(station))
            ...
            WMCChannel preferredChannel = null;
            ...
            foreach (WMCChannel channel in WMCChannel.Channels)
            {
                if (station.Name == channel.CallSign)
                {
                    ...
                    {
                        if (!string.IsNullOrWhiteSpace(channel.Uid))
                        {
                            if (channel.Uid.ToLowerInvariant().Contains("epgcollector"))
                                preferredChannel = channel;
                        }
                    }

                    if (setWMCStationInfo(station, channel, "call sign"))
                    {
                        if (manualMatchList.Contains(station))
                            manualMatchList.Remove(station);
                        return;
            ...
            foreach (WMCChannel channel in WMCChannel.Channels)
            {
                foreach (WMCTuningInfo tuningInfo in channel.TuningInfos)
                {
                    if (tuningInfo as WMCDVBTuningInfo != null)
                    {
                        if (station.OriginalChannelNumber == channel.ChannelNumber)
                        {
                            if (setWMCStationInfo(station, channel, "channel number"))
                            {
                                if (manualMatchList.Contains(station))
                                    manualMatchList.Remove(station);
                                return;
                    ...
                    {
                        if (tuningInfo as WMCATSCTuningInfo != null)
                        {
                            if (station.TransportStreamID + "." + station.ServiceID == channel.ChannelNumber.ToString())
                            {
                                if (setWMCStationInfo(station, channel, "channel number and sub-number"))
                                {
                                    if (manualMatchList.Contains(station))
                                        manualMatchList.Remove(station);
                                    return;
                        ...
                        {
                            if (tuningInfo as WMCDigitalCableTuningInfo != null)
                            {
                                if (station.ServiceID == channel.ChannelNumber)
                                {
                                    if (setWMCStationInfo(station, channel, "channel number"))
                                    {
                                        if (manualMatchList.Contains(station))
        ...
        private bool setWMCStationInfo(TVStation station, WMCChannel channel, string matchMethod)
        {
            if (OptionEntry.IsDefined(OptionName.AutoMapEpg))
            {
                station.WMCUniqueID = channel.Uid;
                station.WMCMatchName = channel.MatchName;
                station.WMCCallSign = channel.CallSign;
            }
        ...
        {
            if (!OptionEntry.IsDefined(OptionName.CreateSatIni))
                return;
            ...
            if (string.IsNullOrWhiteSpace(RunParameters.Instance.SatelliteDefFilesDirectory))
                directory = RunParameters.DataDirectory;
            else
                directory = RunParameters.Instance.SatelliteDefFilesDirectory;

            foreach (SatelliteEntry satelliteEntry in satelliteEntries)
            { 
                string fileName = Path.Combine(directory, satelliteEntry.OrbitalPosition.ToString().PadLeft(4, '0') + ".ini");
