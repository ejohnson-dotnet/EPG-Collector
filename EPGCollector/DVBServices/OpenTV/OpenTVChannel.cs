...
namespace DVBServices
{
    ...
    public class OpenTVChannel : Channel
    {
        ...
        public void ProcessChannelForEPG(TVStation station, Logger titleLogger, Logger descriptionLogger, Logger extendedDescriptionLogger, Logger undefinedRecordLogger)
        {
            ...
            foreach (OpenTVTitleData titleData in TitleData)
            {
                ...
                if (string.IsNullOrWhiteSpace(epgEntry.ShortDescription))
                    epgEntry.ShortDescription = RunParameters.NoProgrammeDescription;
        ...
        private void getEventCategory(OpenTVTitleData titleData, EPGEntry epgEntry)
        {
            ...
            {
                ProgramCategory overrideCategory = getCustomCategory(epgEntry.EventName, epgEntry.ShortDescription);
                if (overrideCategory != null)
                    epgEntry.EventCategory = new EventCategorySpec(overrideCategory);
            }
            ...
            ProgramCategory category = OpenTVProgramCategory.FindCategory(titleData.CategoryID);
            if (category != null)
            {
                epgEntry.EventCategory = new EventCategorySpec(category);
                if (category.SampleEvent == null)
                    category.SampleEvent = epgEntry.EventName;
                category.UsedCount++;
                return;
            }
            else
                OpenTVProgramCategory.AddUndefinedCategory(titleData.CategoryID, epgEntry.EventName);

            if (OptionEntry.IsDefined(RunParameters.Instance.CurrentFrequency.AdvancedRunParamters.Options,
                OptionName.CustomCategoryOverride))
                return;

            ProgramCategory customCategory = getCustomCategory(epgEntry.EventName, epgEntry.ShortDescription);
        ...
        private void getSkyGBRSeasonEpisode(EPGEntry epgEntry)
        {
            try
            {
                if (getSkyGBREpisodeFormat10(epgEntry))
                    return;
                ...
                index1 = epgEntry.ShortDescription.IndexOf("(S");
                if (index1 == -1)
                    index1 = epgEntry.ShortDescription.IndexOf("( S");
                if (index1 != -1)
                {
                    getSkyGBREpisodeFormat7(epgEntry, index1);
                ...
                if (index1 != -1)
                {
                    getSkyGBREpisodeFormat8(epgEntry, index1);
                    return;
                }
                ...
                if (index1 != -1)
                {
                    getSkyGBREpisodeFormat9(epgEntry, index1);
                    return;
                }
        ...
        private void getSkyGBREpisodeFormat7(EPGEntry epgEntry, int index1)
        {
            int startIndex = index1;

            if (index1 + 2 >= epgEntry.ShortDescription.Length)
                return;

            if (epgEntry.ShortDescription[index1 + 1] == ' ')
                index1++;

            if (epgEntry.ShortDescription[index1 + 1] != 'S')
                return;

            index1 += 2;
            int seasonNumber = 0;

            while (index1 < epgEntry.ShortDescription.Length && Char.IsDigit(epgEntry.ShortDescription[index1]))
            {
                seasonNumber = (seasonNumber * 10) + (epgEntry.ShortDescription[index1] - '0');
                index1++;
            }

            if (index1 == epgEntry.ShortDescription.Length)
                return;

            if (epgEntry.ShortDescription[index1] != '/' && epgEntry.ShortDescription[index1] != ' ' && epgEntry.ShortDescription[index1] != 'E')
                return;

            if (epgEntry.ShortDescription[index1] == '/')
            {
                index1++;
                if (index1 == epgEntry.ShortDescription.Length)
                    return;

                if (epgEntry.ShortDescription[index1] == 'E')
                    index1++;
                else
                {
                    if (epgEntry.ShortDescription[index1] == ' ')
                    {
                        index1++;
                        if (index1 == epgEntry.ShortDescription.Length || epgEntry.ShortDescription[index1] != 'E')
                            return;
                ...
                if (epgEntry.ShortDescription[index1] == 'P')
                {
                    index1++;
                    if (index1 == epgEntry.ShortDescription.Length)
                        return;
            ...
            else
            {
                if (epgEntry.ShortDescription[index1] == ' ')
                {
                    ...
                    if (epgEntry.ShortDescription[index1] != 'e' && epgEntry.ShortDescription[index1] != 'E')
                        return;
            ...
            while (index1 < epgEntry.ShortDescription.Length && Char.IsDigit(epgEntry.ShortDescription[index1]))
            {
                episodeNumber = (episodeNumber * 10) + (epgEntry.ShortDescription[index1] - '0');
                index1++;
            }
            ...
            if (epgEntry.ShortDescription[index1] != ')' && epgEntry.ShortDescription[index1] != '/')
                return;

            setSeriesEpisode(epgEntry, seasonNumber, episodeNumber);

            if (!OptionEntry.IsDefined(OptionName.NoRemoveData))
            {
                if (epgEntry.ShortDescription[index1] == '/')
                {
                    while (index1 < epgEntry.ShortDescription.Length && epgEntry.ShortDescription[index1] != ')')
                        index1++;
                ...
                int length = index1 - startIndex;
                epgEntry.ShortDescription = epgEntry.ShortDescription.Remove(startIndex, length + 1).Trim();                
            }
        }

        private void getSkyGBREpisodeFormat8(EPGEntry epgEntry, int index1)
        {
            int startIndex = index1;

            if (index1 + 4 >= epgEntry.ShortDescription.Length)
                return;
            ...
            setSeriesEpisode(epgEntry, -1, episodeNumber);
        ...
        private void getSkyGBREpisodeFormat9(EPGEntry epgEntry, int index1)
        {
            if (index1 < 2)
            {
                if (DebugEntry.IsDefined(DebugName.EpisodeResult))
                    Logger.Instance.Write("Format 9: " + epgEntry.ShortDescription + "= failed");
                return;
            }
            ...
            if (index3 >= epgEntry.ShortDescription.Length)
                return;            
            if (!char.IsDigit(epgEntry.ShortDescription[index3]))
                return;
            ...
            if (epgEntry.ShortDescription[index4] == ' ')
                index4++;
            if (!char.IsDigit(epgEntry.ShortDescription[index4]))
                return;
            ...
            if (DebugEntry.IsDefined(DebugName.EpisodeResult))
                Logger.Instance.Write("Format 9: " + epgEntry.ShortDescription.Substring(index2, index5 - index2) + "= season " + series + " episode " + episode);
        ...
        private bool getSkyGBREpisodeFormat10(EPGEntry epgEntry)
        {
            int startIndex = 0;

            while (startIndex != -1)
            {
                if (format10LocateStart(epgEntry.ShortDescription, ref startIndex))
                {
                    if (format10ProcessEntry(epgEntry, startIndex))
                        return true;
                }
            }
        ...
        private bool format10LocateStart(string description, ref int startIndex)
        {
            startIndex = description.IndexOf("S", startIndex);
            if (startIndex == -1)
                return false;
            ...
            if (char.IsDigit(description[startIndex]))
                return true;
            else
                return false;
        }

        private bool format10ProcessEntry(EPGEntry epgEntry, int startIndex)
        {
            int seriesStart = startIndex;
 
            int series = 0;

            while (startIndex < epgEntry.ShortDescription.Length && char.IsDigit(epgEntry.ShortDescription[startIndex]))
            {
                series = (series * 10) + (epgEntry.ShortDescription[startIndex] - '0');
                startIndex++;
            }

            if (startIndex == epgEntry.ShortDescription.Length)
                return false;
            if (series == 0)
                return false;
            ...
            while (startIndex < epgEntry.ShortDescription.Length && char.IsDigit(epgEntry.ShortDescription[startIndex]))
            {
                episode = (episode * 10) + (epgEntry.ShortDescription[startIndex] - '0');
                startIndex++;
            }
            ...
            {
                seriesStart--;
                if (epgEntry.ShortDescription[seriesStart] == ' ')
                    seriesStart--;

                if (seriesStart > 0)
                {
                    if (epgEntry.ShortDescription[seriesStart - 1] == '(')
                        seriesStart--;
                }
                ...
                if (endIndex == epgEntry.ShortDescription.Length)
                    epgEntry.ShortDescription = epgEntry.ShortDescription.Remove(seriesStart);
                else
                    epgEntry.ShortDescription = epgEntry.ShortDescription.Remove(seriesStart, endIndex - seriesStart + 1).Trim();

                /*int length;
                if (startIndex != epgEntry.ShortDescription.Length)
                    length = startIndex - seriesStart + 1;
                else
                    length = startIndex - seriesStart;
                epgEntry.ShortDescription = epgEntry.ShortDescription.Remove(seriesStart, length).Trim();*/
            }
        ...
        private void getSkyNZLDirector(EPGEntry epgEntry)
        {
            ...
            epgEntry.Directors.Add(new Person(epgEntry.ShortDescription.Substring(startIndex + searchString.Length, stopIndex - (startIndex + searchString.Length)), "Director"));
        ...
        private int addCastName(Collection<Person> cast, string description, int startIndex)
        {
            ...
            cast.Add(new Person(description.Substring(startIndex, castStopIndex - startIndex).Trim(), "Actor"));
        ...
        private void getSkyAUSGBRSubTitle(EPGEntry epgEntry)
        {
            ...
            if (colonIndex != -1)
            {
                if (colonIndex < epgEntry.ShortDescription.Length / 3)
                {
