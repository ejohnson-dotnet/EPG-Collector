    ...
    public class BellTVSection
    {
        ...
        public void Process(byte[] byteData, Mpeg2ExtendedHeader mpeg2Header)
        {
            ...
            while (lastIndex < byteData.Length - 4)
            {
                ...
                if (bellTVEntry.Cast != null && bellTVEntry.Cast.Count != 0)
                {
                    epgEntry.Cast = new Collection<Person>();
                    foreach (string castMember in bellTVEntry.Cast)
        ...
        private EventCategorySpec getEventCategory(string title, string description, int contentType, int contentSubType)
        {
            ...
            ProgramCategory mainCategoryEntry = BellTVProgramCategory.FindCategory(contentType, 0);
            if (mainCategoryEntry == null)
            {
                BellTVProgramCategory.AddUndefinedCategory(contentType, contentSubType, title);
