    ...
    public class DishNetworkSection
    {
        ...
        public void Process(byte[] byteData, Mpeg2ExtendedHeader mpeg2Header)
        {
            ...
            while (lastIndex < byteData.Length - 4)
            {
                ...
                if (dishNetworkEntry.Cast != null && dishNetworkEntry.Cast.Count != 0)
                {
                    epgEntry.Cast = new Collection<Person>();
                    foreach (string castMember in dishNetworkEntry.Cast)
        ...
        private EventCategorySpec getEventCategory(string title, string description, int contentType, int contentSubType)
        {
            ...
            {
                ProgramCategory notPresentCategory = getCustomCategory(title, description);
                if (notPresentCategory != null)
                    return new EventCategorySpec(notPresentCategory);
                else
                    return (null);
            }
            ...
            if (contentType == 0)
            {
                ProgramCategory categoryEntry = DishNetworkProgramCategory.FindCategory(0, contentSubType);
                if (categoryEntry == null)
                {
                    DishNetworkProgramCategory.AddUndefinedCategory(0, contentSubType, title);
                ...
                return new EventCategorySpec(categoryEntry);
            }

            ProgramCategory mainCategoryEntry = DishNetworkProgramCategory.FindCategory(contentType, 0);
            if (mainCategoryEntry == null)
            {
                DishNetworkProgramCategory.AddUndefinedCategory(contentType, 0, title);
                ...
                OptionName.UseContentSubtype))
            {
                ...
                return new EventCategorySpec(mainCategoryEntry);
            }

            ProgramCategory subCategoryEntry = DishNetworkProgramCategory.FindCategory("0," + contentSubType);
            ...
            DishNetworkProgramCategory generatedCategory = new DishNetworkProgramCategory();
            generatedCategory.XmltvDescription = mainCategoryEntry.XmltvDescription + " - " + subCategoryEntry.XmltvDescription;
            generatedCategory.WMCDescription = mainCategoryEntry.WMCDescription + "," + subCategoryEntry.WMCDescription;
            
            return new EventCategorySpec(generatedCategory);
        }
