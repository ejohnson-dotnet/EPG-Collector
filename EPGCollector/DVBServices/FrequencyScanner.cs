    ...
    public class FrequencyScanner
    {
        ...
        public Collection<TVStation> FindTVStations()
        {
            if (dataProvider.Frequency.TunerType != TunerType.ATSC && dataProvider.Frequency.TunerType != TunerType.ATSCCable)
                ...
                return (findAtscStations(dataProvider.Frequency.TunerType));
        }
        ...
        private void processServiceDescriptionSection(ServiceDescriptionSection serviceDescriptionSection, Collection<TVStation> tvStations, TuningFrequency frequency, bool thisFrequency)
        {
            foreach (ServiceDescription serviceDescription in serviceDescriptionSection.ServiceDescriptions)
            {
                ...
                if (processStation)
                {
                    ...
                    tvStation.StationType = TVStationType.Dvb;
                    tvStation.TunerType = frequency.TunerType;

                    if (DebugEntry.IsDefined(DebugName.LogStationType))
                        Logger.Instance.Write("Station " + tvStation.FullDescription + " tuner type set to " + tvStation.TunerType);
        ...
        private Collection<TVStation> findAtscStations(TunerType tunerType)
        {
            ...
            foreach (VirtualChannel channel in VirtualChannelTable.Channels)
            {
                ...
                station.AtscTSID = channel.TransportStreamID;
                station.AtscProgramNumber = channel.ProgramNumber;
                station.AtscHidden = channel.Hidden;
                station.AtscAccessControlled = channel.AccessControlled;
                station.AtscOutOfBand = channel.OutOfBand;
                station.AtscGuideHidden = channel.HideGuide;
                station.AtscServiceType = channel.ServiceType;
