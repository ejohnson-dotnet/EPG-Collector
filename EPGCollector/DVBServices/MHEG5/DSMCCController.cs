...
namespace DVBServices
{
    ...
    public class DSMCCController : ControllerBase
    {
        ...
        public override void FinishFrequency()
        {
            ...
            logChannelInfo();
            OutputFileChannelDefinitions.Process();
        }
                        ...
                        if (pngBinding.IOPIOR != null)
                        {
                            ...
                            if (pngFile != null)
                            {
                                if (pngBinding.Names[0].Identity[0] >= '0' && pngBinding.Names[0].Identity[0] <= '9')
                                {
                                    ...
                                    try
                                    {
                                        string logoPath = Path.Combine(RunParameters.DataDirectory, "Images") + Path.DirectorySeparatorChar;
                                        if (!Directory.Exists(logoPath))
                                            Directory.CreateDirectory(logoPath);

                                        string outFile = logoPath + pngBinding.Names[0].Identity;
                        ...
                        try
                        {
                            if (dataFields[1].Length != 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    ProgramCategory customCategory = CustomProgramCategory.FindCategory(epgEntry.EventName);
                                    if (customCategory == null)
                                        customCategory = CustomProgramCategory.FindCategory(epgEntry.ShortDescription);
                                    if (customCategory != null)
