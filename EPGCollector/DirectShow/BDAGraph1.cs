...
namespace DirectShow
{
    ...
    public class BDAGraph : DirectShowGraph, ITunerDataProvider, ISampleDataProvider
    {
        ...
        {
            IMPEG2PIDMap pidMapInterface = getPidMapInterfaces();
            ...
            if (psiPids != null)
            {
                ...
                if (pidMapInterface != null)
                {
                    if (psiPids.Length != 1 || (psiPids.Length == 1 && psiPids[0] != -1))
                    {
                        if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                            Logger.Instance.Write("BDA PID Mapping unmapping pids");
                        DsError.ThrowExceptionForHR(pidMapInterface.UnmapPID(psiPids.Length, psiPids));
                    }
                }
            }
            ...
            if (pidMapInterface != null)
            {
                if (newPids.Length != 1 || (newPids.Length == 1 && newPids[0] != -1))
                {
                    if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                        Logger.Instance.Write("BDA PID Mapping mapping pids");
                    DsError.ThrowExceptionForHR(pidMapInterface.MapPID(newPids.Length, newPids, MediaSampleContent.TransportPacket));
                }
            }
            ...
            if (pidMapInterface != null)
                releasePidMapInterfaces(pidMapInterface);
        }

        private void logPidsUnMapped(int[] oldPids)
        {
            if (oldPids.Length == 0)
                return;
        ...
        private IMPEG2PIDMap getPidMapInterfaces()
        {
            IBDA_Topology topology = tunerFilter as IBDA_Topology;
            //IBDA_Topology topology = mpeg2DemuxFilter as IBDA_Topology;
            if (topology == null)
            {
                if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                    Logger.Instance.Write("BDA PID Mapping can't get topology");
                return (null);
            }

            if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                Logger.Instance.Write("BDA PID Mapping topology available");
            ...
            if (nodeTypeCount != 0)
            {
                if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                {
                    ...
                    Logger.Instance.Write("BDA PID Mapping node types: " + displayNodes);
                }
            }
            else
            {
                if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                    Logger.Instance.Write("BDA PID Mapping GetNodeTypes returned zero entries");
                return null;
            }
            ...
            if (descriptorCount != 0)
            {
                ...
                {
                    for (int index = 0; index < descriptorCount; index++)
                        Logger.Instance.Write("BDA PID Mapping descriptor " + descriptors[index].guidName.ToString() + " " + descriptors[index].guidFunction.ToString() + " " + descriptors[index].ulBdaNodeType);
                }
            }
            else
            {
                if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                    Logger.Instance.Write("BDA PID Mapping GetNodeDescriptors returned zero entries");
                return null;
            }

            for (int nodeIndex = 0; nodeIndex < nodeTypeCount; nodeIndex++)
            {
                int nodeType = nodeTypes[nodeIndex];

                if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                    Logger.Instance.Write("BDA PID Mapping processing node type " + nodeType);
                ...
                for (int descriptorIndex = 0; descriptorIndex < descriptorCount; descriptorIndex++)
                {
                    ...
                    {
                        if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                            Logger.Instance.Write("BDA PID Mapping processing descriptor with node type " + nodeType);

                        if (descriptor.guidFunction == BDANodeCategory.PidFilter)
                        {
                            if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                                Logger.Instance.Write("BDA PID Mapping found pid mapping descriptor");

                            int interfaceCount;
                            Guid[] interfaces = new Guid[32];
                            reply = topology.GetNodeInterfaces(nodeTypes[nodeIndex], out interfaceCount, 32, interfaces);
                            DsError.ThrowExceptionForHR(reply);

                            if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                            {
                                Logger.Instance.Write("BDA PID Mapping descriptor has " + interfaceCount + " interfaces");

                                for (int logIndex = 0; logIndex < interfaceCount; logIndex++)
                                    Logger.Instance.Write("BDA PID Mapping descriptor interface GUID " + interfaces[logIndex]);
                            }

                            for (int searchIndex = 0; searchIndex < interfaceCount; searchIndex++)
                            {
                                /*if (interfaces[searchIndex] == typeof(IMPEG2PIDMap).GUID)
                                {*/
                                    object controlNode;
                                    reply = topology.GetControlNode(0, 1, nodeTypes[nodeIndex], out controlNode);
                                    DsError.ThrowExceptionForHR(reply);

                                    IMPEG2PIDMap pidMapInterface = controlNode as IMPEG2PIDMap;
                                    if (pidMapInterface == null)
                                    {
                                        if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                                            Logger.Instance.Write("BDA PID Mapping can't get interface: cast of control node failed");
                                    }
                                    else
                                    {
                                        if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                                            Logger.Instance.Write("BDA PID Mapping returning interface");
                                        return pidMapInterface;
                                    }
                                /*}*/
                            }
                        }
            ...
            if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                Logger.Instance.Write("BDA PID Mapping interface not found - no pid mapping available");

            return null;
        }

        private void releasePidMapInterfaces(IMPEG2PIDMap pidMap)
        {
            if (TraceEntry.IsDefined(TraceName.BdaPidMap))
                Logger.Instance.Write("BDA Releasing pid map interfaces");

            Marshal.ReleaseComObject(pidMap);
        }
    }
}