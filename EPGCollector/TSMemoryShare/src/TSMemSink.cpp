	...
	dumpPath()
{
	...
	if (writeDumpFile == 1)
	{
		...
		dumpPath = (char*)malloc(strlen(dumpFileName));
		strcpy(dumpPath, dumpFileName);			
	}
...
HRESULT CTSMemSinkFilter::OpenDumpFile(char* fileName)
{
    ...
    if (m_hDumpFile == INVALID_HANDLE_VALUE) 
	{
		LogString("Failed to open dump file\r\n");
		writeDumpFile = false;
		return (S_FALSE);
	}

	LogString("Dump file opened\r\n");	
	return (S_OK);
}
...
STDMETHODIMP CTSMemSinkFilter::Run(REFERENCE_TIME tStart)
{
	...
	if (writeDumpFile && m_hDumpFile == INVALID_HANDLE_VALUE)
		OpenDumpFile(dumpPath);
