    ...
    public class XmltvTime
    {
        ...
        private void load(string timeData)
        {
            ...
            try
            {
                ...
                Time = DateTime.ParseExact(timeParts[0], "yyyyMMddHHmmss", CultureInfo.InvariantCulture);
            }
            catch (Exception)
            {
                try
                {
                    Time = DateTime.ParseExact(timeParts[0], "yyyy-MM-dd", CultureInfo.InvariantCulture);
                }
                catch (Exception e)
                {
                    Logger.Instance.Write("<E> An exception of type " + e.GetType().Name + " has occurred while parsing a date field");
            ...
            else
            {
                ...
                {
                    Logger.Instance.Write("<E> An exception of type " + e.GetType().Name + " has occurred while parsing a time field");
                    Logger.Instance.Write("<E> " + e.Message);
                    return;
                }
            }
