...
namespace Rtsp
{
    ...
    public class RtspOptions : RtspMessageBase
    {
        ...
        public ErrorSpec Process(string hostAddress, int hostPort, int cseq, string sessionId, int streamId)
        {
            ...
            SatIpLogger.Instance.Write("Sending OPTIONS message to " + hostAddress + " port " + hostPort);

            TcpClient tcpClient = new TcpClient(hostAddress, hostPort);
            NetworkStream networkStream = tcpClient.GetStream();

            byte[] requestMessage = outputStream.ToArray();
            SatIpLogger.Instance.LogReply("RTSP OPTIONS request: ", requestMessage, requestMessage.Length);
            ...
            SatIpLogger.Instance.LogReply("RTSP OPTIONS response: ", responseMessage, byteCount);  
