...
namespace UPnP
{
    ...
    public class NotifyMessage : UPnPMessage
    {
        ...
        public ErrorSpec Receive(Socket socket, int timeout, string notificationType)
        {
            ...
            SatIpLogger.Instance.LogReply("Notify message: ", inputBuffer, receiveCount);            

            ErrorSpec reply = Process(inputBuffer, receiveCount);
            if (reply != null)
                return (reply);
            
            SatIpLogger.Instance.Write("Notify message: Processed successfully");
            ...
            SatIpLogger.Instance.Write("Notify message: Loading description");

            WebClient webClient = new WebClient();
            byte[] response = webClient.DownloadData(Location);

            SatIpLogger.Instance.LogReply("Notify description: ", response, response.Length); 
            ...
            if (newDevice && Description.Root.Devices != null)
            {
                foreach (Device device in Description.Root.Devices)
                    SatIpLogger.Instance.Write("Notify device: " + device.FriendlyName + " loaded");
            }
