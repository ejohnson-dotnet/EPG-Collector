...
namespace UPnP
{
    ...
    public class MSearchMessage : UPnPMessage
    {
        ...
        public ErrorSpec Send(IPAddress address, int port, Socket socket, string searchTarget)
        {
            ...
            SatIpLogger.Instance.LogReply("M-SEARCH request: ", requestMessage, requestMessage.Length);
        ...
        private ErrorSpec Receive(Socket socket, int timeout)
        {
            ...
            SatIpLogger.Instance.LogReply("M-SEARCH response: ", inputBuffer, receiveCount);
        ...
        public ErrorSpec Process(byte[] buffer, int count)
        {
            ...
            SatIpLogger.Instance.Write("M-SEARCH response: Loading description");
            ...
            SatIpLogger.Instance.LogReply("M-SEARCH description: ", response, response.Length);
        ...
        /// <returns>An ErrorSpec instance if an error occurs; null otherwise.</returns>
        public static ErrorSpec SearchForServers(string searchTarget, Collection<UPnPMessage> responses, string uuid, string type)
        {
            SatIpLogger.Instance.Write("Searching for " + type + " servers");
            ...
            {
                SatIpLogger.Instance.Write("<e> Failed to create socket");
            ...
            while (reply == null)
            {
                ...
                if (reply == null)
                {
                    /*if (!searchMessage.Server.ToUpperInvariant().Contains("WINDOWS") &&
                        (searchTarget == null || (searchMessage.SearchTarget.StartsWith(searchTarget))))
                ...
                {
                    SatIpLogger.Instance.Write("<e> Server search error: " + reply);
                    if (reply.OriginalErrorCode == -1)
                    {
                        SatIpLogger.Instance.Write("<e> Server response ignored");
                        reply = null;
                    }
                }
            ...
            SatIpLogger.Instance.Write("Searching for " + type + " servers returning " + responses.Count);
        ...
        private static bool checkServer(MSearchMessage searchMessage, string searchTarget, string uuid)
        {
            ...
            if (searchTarget != null && searchMessage.SearchTarget.StartsWith(searchTarget))
                return (true);
            if (uuid != null && searchMessage.UniqueServiceName.Contains(uuid))
                return (true);
        ...
        {
            IPAddress localIPAddress = Utils.GetLocalIpAddress(SatIpLogger.Instance);
            if (localIPAddress == null)
                throw (new InvalidOperationException("No IPv4 address located to set the local IP address"));

            SatIpLogger.Instance.Write("Local IP address set to " + localIPAddress);
            ...
            SatIpLogger.Instance.Write("UPnP client port set to " + UPnPClientPort);
